...
| --- | --- | --- | --- |
| `hermes/` | Hermes replacement, migration, profile, behavior, and ownership policy | Production Hermes deployment and recovery tooling | Credential-free structured migration/behavior contracts, the exhaustive Ansible path-ownership contract, and root-owned Astra, Dubble, and Rigel security baselines. Mutable native state is classified separately from rebuildable platform and explicit restore state. |
