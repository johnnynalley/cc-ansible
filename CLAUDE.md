...
- **Local (ts440 ZFS)**: Hourly via `local-restic.yml`. Backs up `/opt` from VMs to `/srv/nas-zfs/backups/<hostname>/`. Retention: 24h/7d/4w/6m. Uses dedicated SSH key in `group_vars/backup_clients/vault.yml`. The backup key on ts440 is restricted to SFTP-only via `command="/usr/lib/openssh/sftp-server",restrict` in authorized_keys. Hosts without Tailscale SSH access (e.g., openclaw-vm) connect on port 2222 (`local_restic_ssh_port` in host_vars) to bypass Tailscale SSH interception; ts440's sshd listens on both 22 and 2222 via drop-in config.
