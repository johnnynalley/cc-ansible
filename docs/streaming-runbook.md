...
- Landscape Twitch and YouTube are handled by `stream-relay.service` on `media-vm`.
...
Confirm the relay and TikTok broker are up:

```bash
ansible media-vm -m shell -a "systemctl is-active stream-relay.service stream-relay-vertical-broker.service" --become
```
...
Current relay outputs are controlled by:

```text
STREAM_RELAY_OUTPUTS="twitch youtube"
```

in `/etc/stream-relay/stream-relay.env`.
...
3. Stop the media-vm relay if desired:

```bash
ansible media-vm -m shell -a "systemctl stop stream-relay.service" --become
```
