...
| `docs/immich-media-inbox.md` | Immich screenshot media discovery | Candidate detection, OCR/title matching, Astra result policy, API permissions, Seerr request gating, deployment, or rollback changes. | `playbooks/media/immich-media-inbox.yml`, `scripts/media-inbox/`, `templates/media-inbox/` |
...
| `docs/openclaw-runtime-security.md` | OpenClaw OS identities and trust boundaries | Gateway/receiver identities, raw-versus-aggregate data access, canary/cutover flow, prompt-injection containment, or credential reach changes. | `playbooks/agents/openclaw-health-receiver.yml`, `playbooks/agents/openclaw-isolated-gateway.yml`, `scripts/agents/`, `templates/openclaw/`, `docs/openclaw-docker-access.md` |
