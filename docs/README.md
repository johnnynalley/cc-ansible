...
| `docs/plex-appliance-operations.md` | Plex appliance operator actions | Bedroom/living-room Plex appliance host mapping, Plex server storage backing, playback identity/session handling, queue safety, HDMI display ownership, skip-current-episode steps, shuffle-state rollback handling, or appliance operator commands change. | `playbooks/media/plex-appliance.yml`, `templates/plex-appliance/`, `inventory/host_vars/*/plex-appliance.yml`, `inventory/host_vars/*/nfs.yml`, `inventory/host_vars/media-vm/` |
