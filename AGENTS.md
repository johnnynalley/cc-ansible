...
Self-evolution must not wait for the user to name the failure. Before closing a task where Codex notices it used a slower path, missed an established source of truth, repeated avoidable work, needed a user correction for an already-documented behavior, or found a better general rule during execution, pause for a brief after-action check. If the issue is likely to recur, persist the rule or runbook update immediately, then commit shared guidance changes before final response.
