...
**Immich Media Inbox**: `docs/immich-media-inbox.md` is the source of truth for discovering movie/TV screenshots through Immich OCR and Smart Search. The service is headless and Astra-operated through a strict result-only wrapper: do not add a Web UI, expose image/thumbnail bytes or raw OCR, grant Astra direct Immich/SQLite/Docker/credential access, or bypass ambiguity and explicit-request gates. Initial calibration remains review-only with Seerr requests disabled.
