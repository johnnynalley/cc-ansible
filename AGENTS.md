...
**Critical**: All media containers must use the same in-container path (`/data`) backed by the responsive Plex-library VirtioFS mount (`/srv/plex:/data` on media-vm). Do not switch them back to `/srv/media/plex:/data` just for path symmetry: on 2026-05-24 the whole-media `/srv/media` VirtioFS mount was hanging while the dedicated `/srv/plex` mount remained healthy. Hardlinks still work because completed downloads and media libraries share the same `/data` mount; this was verified with a real `ln` test across `/data/downloads/complete` and `/data/Anime`.
