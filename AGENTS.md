...
`README.md` files inside `templates/<domain>/` are repository catalogs, not rendered deployment sources. Do not describe or treat them as files Ansible deploys unless a task actually references them as `src:`. For OpenClaw specifically, keep ownership boundaries explicit: Ansible may manage package/runtime prerequisites, service units and drop-ins, and reusable host-side support scripts, but it must not copy, template, replace, or periodically restore agent-authored workspace state such as `AGENTS.md`, `MEMORY.md`, `SOUL.md`, `HEARTBEAT.md`, `TOOLS.md`, `USER.md`, `SKILL.md`, or workspace README files unless the user explicitly approves a source-of-truth migration. Before adding any OpenClaw `copy`, `template`, sync, or restore task, audit its live destination and confirm that it cannot overwrite Astra's self-maintained workspace changes.

### Documentation Cross-References

When creating or materially updating operator docs, policy docs, runbooks, or troubleshooting guides under `docs/`, update `docs/README.md` and add or update the matching pointer in `AGENTS.md` in the relevant operational section. The point is discoverability: future agents should know where to find the source of truth without guessing filenames or relying on memory. Current source-of-truth docs include `docs/capture-card-streaming-plan.md`, `docs/fortnite-performance-investigation.md`, `docs/gaming-benchmark.md`, `docs/hermes-replacement.md`, `docs/immich-media-inbox.md`, `docs/media-release-policy.md`, `docs/agent-docker-access.md`, `docs/openclaw-heartbeats.md`, `docs/openclaw-runtime-security.md`, `docs/plex-appliance-operations.md`, and `docs/streaming-runbook.md`. If a new doc captures behavior that should persist across sessions, also add a concise Codex memory note when the user explicitly asks for memory persistence.
...
Treat Codex guidance and memory as moving operating state, not static background text. When a repeated correction, stale path, stale command, missing recurring workflow, broken plan handoff, explicit user instruction, or Codex's own observed execution failure shows that Codex should preserve a behavior change, update the appropriate durable source in the same turn: `AGENTS.md` for repo/workflow rules, a repo doc/runbook for operational procedure, and a Codex memory note when the user explicitly authorizes memory persistence or asks memory to be updated. Do not rely on a chat promise alone.

This is an immediate trigger, not just a scheduled audit. If the need is discovered while working, persist the rule before continuing unrelated work unless doing so would be unsafe or would collide with another active edit; in that case, state the blocker and the exact hunk that still needs to land.

Self-evolution must not wait for the user to name the failure. Before closing a task where Codex notices it used a slower path, missed an established source of truth, repeated avoidable work, needed a user correction, or found a better general rule during execution, pause for a brief after-action check. Ask: why was correction needed, what existing rule failed or was missing, and what durable change would prevent the same miss. If the issue is likely to recur, persist the rule or runbook update immediately, then commit shared guidance changes before final response.
...
Before classifying mount drift, backup-target failure, or application storage
...
model-visible source skill, enabled schedule, retained data set, integration,
and action-level tool capability. Validate each retained user workflow as the
Hermes service identity inside its hardened runtime namespace; a retained file
without the skill, tool, credentials, permissions, or delivery path needed to
use it is not parity. Record product-specific retirements explicitly with the
...
second action statement; task listing forces the fuller parser without making
live changes.
...
When reverting an unapproved or experimental live path change, restore the old
runtime behavior and clean up the compatibility state in the same session. Do
not leave temporary mounts, alternate completed-download paths, moved payloads,
or "legacy drain" comments as unresolved debt unless the user explicitly asks
to pause cleanup. If completed payloads already landed in the temporary path,
move or import them back to the approved path, verify no live queue/history
still depends on the temporary path, then remove the managed mount/config and
commit the repo cleanup immediately.
...
When alerts, cron failures, service failures, watchdog noise, repeated warnings, or "this is annoying/spammy/broken" reports are involved, root-cause classification is mandatory before mitigation. Do not mute, disable, de-duplicate, restart, retry, re-route, or otherwise quiet the symptom as the primary fix until you identify the owning system, failing check, exact error, last-good state, and recent relevant change. Classify the issue as one of: real outage, stale alert logic, config drift, dependency failure, expected warning, or unknown. Only after that classification may you propose or apply mitigation, and any mitigation remains temporary unless the trigger is understood and prevention is verified.

When an investigation finds something wrong, do not stop at reporting facts or raw audit output. Classify each issue as a policy/config bug, source-availability gap, runtime drift, expected tradeoff, or unknown. Explain the exact mechanism involved, such as the matching rule, score math, import-time reparse, service dependency, or storage behavior. Then propose concrete fix options with impact, risk, verification steps, and the safest next action. If no safe fix is known yet, state the missing evidence and how to collect it.
...
Do not manage Windows GUI-owned consumer settings such as Xbox Game Bar, Game DVR, Windows Settings app toggles, or per-user gaming-app preferences with Ansible registry writes unless the user explicitly approves that exact setting. These GUI surfaces can have app state, policy state, per-user registry state, and Settings UI state that do not map cleanly to one terminal-visible value. Prefer documenting the desired GUI state and letting the user configure it normally. If undoing a prior managed registry tweak, first remove the Ansible source so it stops fighting the GUI, then use only a narrowly scoped one-time cleanup when the user asks for live repair.
...
`/usr/local/bin/storage-status` on TS440. Do not use `df /srv/media` as the
whole answer. That is mergerfs `statfs` for the presented filesystem and can
understate the physical/pool footprint, especially because the ZFS branches use
nested datasets such as `nas_zfs/media/plex`, `media-01/media/plex`, and
`media-02/media/plex`. Report capacity as separate views: `storage-status`,
raw/block devices from `lsblk`, ZFS pool usable capacity and vdev member health
from `zpool list` / `zpool status`, dataset placement from `zfs list -r`,
branch readability from the managed `mergerfs_branches`, and only then the
presented mergerfs view from `df /srv/media`.
...
2. **Mount watchdog** (`mergerfs-mount-watchdog.timer`, every 60s) — backstop that checks every protected branch via `findmnt`. If missing, attempts `systemctl start <mount-unit>`. After 3 consecutive failures (~3 min), sends a loud Apprise alert (`push`). State in `/var/lib/mergerfs-mount-watchdog/`. 6-hour alert dedup so it doesn't spam.
3. **Recovery hook** (`/usr/local/sbin/mergerfs-branch-recovered <name>`) — called by both udev and watchdog after a successful remount. Refreshes Plex (`/library/sections/all/refresh`) on media-vm and Sonarr/Radarr (`RescanSeries` / `RescanMovie`) on docker-vm. Bazarr auto-follows. Sends a quiet Apprise notification (`push-quiet`) with per-API result. ts440 reaches these services directly over Tailscale.
...
**mergerfs-balance**: Balances files across mergerfs branches by moving from the fullest to the emptiest. Default path excludes in `/etc/mergerfs-balance.conf` (deployed by `playbooks/storage/mergerfs.yml` from `mergerfs_balance_exclude_paths` variable) protect irreplaceable data on mirrored nas_zfs (photos, archive, books) from being moved to single-drive pools. CLI `-E` flags are merged with config excludes. `--evacuate <branch>` drains a single branch to the least-used eligible destination branches before planned removal or reformat; dry-run first. Routine nightly balancing is disabled by policy because branch capacity is close enough and nightly Profilarr upgrades should run instead. Exception balance jobs may still be queued through `nightly-media-maintenance balance add ...` and run under `playbooks/media/nightly-media-maintenance.yml`; while pending, a balance job owns the midnight-7 AM window, skips Profilarr for that night, records state under `/var/lib/nightly-media-maintenance/`, uses rsync partial files for next-window resume, and can pause/resume docker-vm `media-stack` while leaving Plex on media-vm up. The timer fires hourly inside the window; `mergerfs-balance` currently scans branches and selects the first eligible file before running the pause hook, so do not assume containers are already stopped at exactly `00:00:00`. For storage-path, hardlink, completed-vs-incomplete download, `/usenet-complete`, or balance/import RCA questions, read `docs/media-stack-storage-layout.md` first and verify live mounts/config before explaining behavior.

**Incomplete Downloads**: The Samsung 860 EVO SSD is mounted directly at `/srv/media-downloads` (defined in `group_vars/nas_server/mounts.yml`) as a dedicated scratch filesystem outside the mergerfs branch tree. docker-vm mounts it over NFS as `/srv/incomplete_downloads`, while media-vm's old VirtioFS mount is disabled with the migrated media automation. The NFS export manages writable temp roots for qBittorrent and SABnzbd under the historical `/srv/media-downloads/incomplete/` layout (`incomplete/torrents`, `incomplete/torrents/tv-sonarr`, `incomplete/torrents/sonarr`, `incomplete/torrents/radarr`, and `incomplete/usenet`) as UID/GID 1000. Completed Arr downloads intentionally stay under `/srv/media/plex` via the container `/data` mount (`/data/downloads/complete` for SABnzbd, `/data/downloads/torrents` for qBittorrent) so Arr imports can hardlink when branch placement allows it. The old Lacie-backed source path `/srv/nas-01/downloads/media-downloads` is a rollback copy after the 2026-06-18 Samsung cutover until cleanup is explicitly approved. If qBittorrent logs `mkdir ... Permission denied` or `missing files` under `/incomplete`, check these NAS-side directory owners and the docker-vm compose bind paths first.

For live storage cutovers, do not use `findmnt -T <path>` by itself as proof that a temporary target is mounted; it can return an ancestor filesystem such as `/`. Use `mountpoint -q <path>` plus `findmnt -n -o SOURCE --target <path>` and verify the expected UUID/by-id source before any rsync or destructive cleanup.
...
**pve-herc shares**: Time Machine (`/srv/pbs-data/timemachine` on the 1TB PBS drive) — defined in `host_vars/pve-herc/samba.yml`. Active macOS Time Machine destination. SMB advertises a 350G Time Machine cap, matching the Mac's `tmutil` destination quota. The sibling PBS chunk store is bounded by an ext4 project quota in `host_vars/pve-herc/mounts.yml` so PBS cannot consume Time Machine's share. SMB port 445 allowed from tailscale0 in pve-herc's firewall.
...
`docs/agent-docker-access.md` is the source of truth for Astra's Docker
inventory and future update path. Never grant the Gateway, its runtime user, or
an agent SSH account membership in the `docker` group or direct access to a
Docker socket; both are root-equivalent. The only approved read architecture is
a root-owned allowlist reporter that writes a bounded redacted result for a
root-owned forced-command account. The reporter feature is disabled until a
dedicated Hermes identity, key, source CIDR, backup, and canary are explicitly
approved. Container updates require the separate immutable plan/approve/apply
broker whose approval occurs outside the agent trust boundary; do not expose
the existing auto-update script or broad `dbc` helpers as Astra tools. Broker
migrations must validate the replacement before activating its key, preserve
prior history as inert root-only data, and invalidate prior pending approvals.
...
**Apprise tags** control routing: `push` (Pushover infrastructure, Time Sensitive), `push-quiet` (Pushover infrastructure, silent), `email` (iCloud SMTP), `media-feed` (Pushover media, silent), and `media-requests` (Seerr media requests, silent). Services specify tags via `apprise_alert_tags` variable (default: `push` in `group_vars/all/vars.yml`). apcupsd supports per-service override via `apcupsd_alert_tags`. Combine tags like `push,email` for multi-target delivery.

**Retired OpenClaw receiver routes**: The Siri `/ask` relay and unauthenticated Apprise `/alerts` receiver were retired on 2026-08-09. Caddy must return `404` for both paths, and managed Apprise tags must not include `dbc`. Do not restore either model-facing ingress. A future alert integration must authenticate unconditionally, enforce a typed bounded schema, and deliver only to a no-exec triage identity.
...
Caddy provides HTTPS for all internal services via Cloudflare DNS-01 challenge. The canonical sources are `templates/docker/Caddyfile.j2` for routes, `templates/docker/caddy.yml` for compose, and `templates/docker/caddy.Dockerfile` for the Cloudflare DNS build. `docker-stacks.yml --tags caddy` renders them under `/opt/caddy/`, validates the Caddyfile, and recreates the Caddy container when the Caddyfile changes. `/opt/caddy/.env` remains live-only because it contains the Cloudflare API token. Cloudflare DDNS records such as `mercury-relay.jnalley.me` are separate and managed by `playbooks/network/cloudflare-ddns.yml` with a dedicated token/env file, not the Caddy `.env`. Tailscale peer-relay static endpoints require literal `IP:port` values, so Mercury's `RelayServerStaticEndpoints` value is kept current by `playbooks/network/tailscale-peer-relay-endpoint.yml`, not by putting the hostname into Tailscale prefs. docker-vm services are proxied by container name (`caddy-proxy` Docker network); media-vm services by Tailscale IP (`100.66.6.113`). Proxmox/PBS/PDM management UIs are proxied at `pve-ts440.jnalley.me`, `pve-alto.jnalley.me`, `pve-herc.jnalley.me`, `pve-m70q.jnalley.me`, `pbs.jnalley.me`, and `pdm.jnalley.me`. All services require Tailscale to access.
...
- **Proxmox Backup Server (PBS)**: VM/CT snapshot backups every 6 hours via `proxmox-backup-server.yml`. pbs-lxc (CT 105 on pve-herc, Debian 13, 4 cores, 2GB RAM) with 1TB ext4 datastore at `/srv/pbs-data`. The same 1TB drive also hosts `/srv/pbs-data/timemachine/` for macOS Time Machine (served by Samba on pve-herc with a 350G advertised cap matching the Mac's `tmutil` destination quota). Registered as `pbs-main` storage on all 4 Proxmox nodes. The job excludes `100,103,104,105,142`; media-vm stays excluded because its large disks do not fit the shared PBS/Time Machine datastore. PBS chunks are capped at 520 GiB with an ext4 project quota on `/srv/pbs-data/.chunks`; keep `prjquota` in the pve-herc mount options. API token auth (`backup@pbs!ansible`, secret in `host_vars/pbs-lxc/vault.yml`). Daily prune job: 6h/3d/2w/1m. **Daily garbage collection** (frees disk space from pruned snapshots — without GC, orphaned chunks accumulate indefinitely). Web UI: `https://100.110.176.37:8007`. PBS dedup makes frequent backups viable — only changed blocks are stored after the initial full backup. Connectivity check runs at `:59` on all nodes (Play 4), logging to journald tag `pbs-check` for Loki.
...
**Proxmox update window**: Proxmox nodes run the weekly full-upgrade and reboot when required. `group_vars/proxmox_nodes/vars.yml` keeps `auto_updates_proxmox_quorum_guard: true`; immediately before scheduling a reboot, the managed script requires a quorate cluster and proves that removing the current node leaves at least the live quorum vote count. It also acquires the shared `/etc/pve/priv/auto-updates-reboot.lock.d` reboot slot, so only one Proxmox node can schedule an automatic reboot at a time even if timers drift or the script is triggered manually on multiple nodes. If either check fails, it blocks the reboot and alerts instead. Per-host `auto_updates_oncalendar` overrides use one-hour non-overlapping slots with `auto_updates_randomized_delay_sec: "0"`: ts440 05:00 → pve-alto 06:00 → pve-herc 07:00 → pve-m70q 08:00. pve-m70q goes last because it hosts docker-vm and several control-plane guests. VMs/LXCs run after the host window (`Sun *-*-* 09:00:00` + 30m random delay), so host reboots do not interrupt guest dpkg transactions. Other Debian hosts use the default schedule from `group_vars/debian_hosts/packages.yml` (`Sun *-*-* 05:00:00` + 15m random delay).
...
**Terminal Arr torrent handoff**: the exact-ID reconciler managed by `playbooks/media/media-release-stamper.yml` may clear only fully classified completed qBittorrent downloads that Arr will not import. It must require exact grab context, complete per-file native mapping, stable read-only payload probes, and a finite existing seed quota. Preserve every per-torrent ratio/time limit, verify qBittorrent's `RemoveWithContent` action by readback before hiding the Arr row with `removeFromClient=false`, and leave the global qBittorrent action unchanged. Ordinary current-better, pack-collateral, superseded, and profile-drift outcomes are never blocklisted; only stable exact identity/payload contradictions may be blocklisted. Any missing evidence or failed write/readback must leave Arr and qBittorrent untouched. The source of truth is `docs/media-release-policy.md` under "Terminal Torrent Validation And Handoff."
...
3. The next time that VM starts (auto or manual), the hookscript checks all required paths via `mountpoint -q` first. Missing → start aborted, Apprise `push` alert fired. All present → start proceeds.
