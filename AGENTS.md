...
**pve-herc shares**: Time Machine (`/srv/pbs-data/timemachine` on the 1TB PBS drive) — defined in `host_vars/pve-herc/samba.yml`. Active macOS Time Machine destination. SMB advertises a 500G Time Machine cap. The sibling PBS chunk store is bounded by an ext4 project quota in `host_vars/pve-herc/mounts.yml` so PBS cannot consume Time Machine's share. SMB port 445 allowed from tailscale0 in pve-herc's firewall.
