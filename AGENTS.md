...
**pve-herc shares**: Time Machine (`/srv/pbs-data/timemachine` on the 1TB PBS drive) — defined in `host_vars/pve-herc/samba.yml`. Active macOS Time Machine destination. SMB normally advertises a 500G Time Machine cap; during the 2026-06 PBS GC bridge it is temporarily capped at 400G while deleted VM100 chunks age out. The sibling PBS chunk store is bounded by an ext4 project quota in `host_vars/pve-herc/mounts.yml` so PBS cannot consume Time Machine's share. SMB port 445 allowed from tailscale0 in pve-herc's firewall.
...
- **Proxmox Backup Server (PBS)**: VM/CT snapshot backups every 6 hours via `proxmox-backup-server.yml`. pbs-lxc (CT 105 on pve-herc, Debian 13, 4 cores, 2GB RAM) with 1TB ext4 datastore at `/srv/pbs-data`. The same 1TB drive also hosts `/srv/pbs-data/timemachine/` for macOS Time Machine (served by Samba on pve-herc, normally with a 500G advertised cap). Registered as `pbs-main` storage on all 4 Proxmox nodes. The job excludes `100,103,104,105,142`; media-vm stays excluded because its large disks do not fit the shared PBS/Time Machine datastore. PBS chunks are capped with an ext4 project quota on `/srv/pbs-data/.chunks`; keep `prjquota` in the pve-herc mount options. During the 2026-06 PBS GC bridge, PBS has a 390 GiB soft target with a temporary 520 GiB hard ceiling and Time Machine is temporarily capped at 400G; restore the normal 390 GiB PBS hard cap and 500G Time Machine cap after PBS `pending-bytes` returns to 0. API token auth (`backup@pbs!ansible`, secret in `host_vars/pbs-lxc/vault.yml`). Daily prune job: 6h/3d/2w/1m. **Daily garbage collection** (frees disk space from pruned snapshots — without GC, orphaned chunks accumulate indefinitely). Web UI: `https://100.110.176.37:8007`. PBS dedup makes frequent backups viable — only changed blocks are stored after the initial full backup. Connectivity check runs at `:59` on all nodes (Play 4), logging to journald tag `pbs-check` for Loki.
