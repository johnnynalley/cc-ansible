...
**media-vm specific files**: stream relay is Ansible-managed (`/usr/local/sbin/stream-relay`, `/usr/local/sbin/stream-relay-output`, `stream-relay.service`, `stream-relay-broker.service`, `stream-relay-output@.service`); qBittorrent port sync is Ansible-managed by `playbooks/gluetun-watchdog.yml` (`/usr/local/bin/qbit-port-sync`, `/etc/qbit-port-sync.env`, systemd path unit `qbit-port-sync.path`); release metadata stamping is Ansible-managed by `playbooks/media-release-stamper.yml` (`/opt/media-stack/qbittorrent/scripts/qbit-release-stamper.py`, `/opt/media-stack/sabnzbd/scripts/sab-release-stamper.py`).
