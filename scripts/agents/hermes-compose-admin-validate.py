#!/usr/bin/env python3
"""Validate Astra's root-managed Compose administration boundary."""
...
def validate_plugin(root: Path) -> list[str]:
    ...
    require(isinstance(metadata, dict) and metadata.get("name") == "compose-admin" and metadata.get("version") == "1.0.0" and metadata.get("module") == "__init__", "plugin-metadata-drift")
    spec = importlib.util.spec_from_file_location("validated_compose_admin", root / "__init__.py")
    ...
    require(names == ["compose_hosts", "compose_request"], "plugin-tool-drift")
    require({item.get("toolset") for item in context.tools} == {"compose_admin"}, "plugin-toolset-drift")
...
def validate_config(path: Path) -> None:
    ...
    require(isinstance(enabled, list) and "compose-admin" in enabled and len(enabled) == len(set(enabled)), "plugin-not-enabled")
    require("compose_admin" in value.get("toolsets", []), "toolset-not-enabled")
    require("compose_admin" not in value.get("agent", {}).get("disabled_toolsets", []), "toolset-disabled")
...
def validate_endpoints(path: Path) -> dict[str, str]:
    ...
    require(isinstance(value, dict) and set(value) == {"schemaVersion", "hosts"} and value.get("schemaVersion") == 1 and isinstance(value.get("hosts"), list), "endpoint-invalid")
    result: dict[str, str] = {}
    for item in value["hosts"]:
        ...
        require(ip_address(item["address"]) in LAN, "endpoint-outside-lan")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(json.dumps({"status": "ok", "plugin": "compose-admin", "hosts": len(endpoints), "tools": tools}, sort_keys=True))
    return 0
