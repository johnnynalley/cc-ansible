...
ANIME_PROFILE_NAME = "movies-anime"
DUAL_AUDIO_CF_NAME = "Anime Dual Audio"
X265_CF_NAME = "x265 (HD)"
QUALITY_RANK_PREFIX = "Local Anime Quality Rank - "
...
def api_get(api_key: str, path: str, params: dict[str, Any] | None = None) -> Any:
    ...
    url = f"{RADARR_URL.rstrip('/')}{path}{query}"
...
def field_value(spec: dict[str, Any], name: str = "value") -> Any:
...
def title_resolution(title: str) -> int | None:
    if re.search(r"(?i)\b(2160p|4k|uhd)\b", title):
        return 2160
    match = re.search(r"(?i)\b(480|576|720|1080)p\b", title)
    if match:
...
def evaluate_spec(spec: dict[str, Any], title: str) -> SpecResult:
    ...
    if implementation in {"ReleaseTitleSpecification", "ReleaseTitleRegexSpecification"}:
        matched = regex_matches(str(field_value(spec) or r"$."), title)
    elif implementation == "LanguageSpecification":
        # The live DA CF's language specs are optional. Radarr evaluates them
        # against parsed release/file language metadata, not only title text.
        matched = False
    elif implementation == "ResolutionSpecification":
        resolution = title_resolution(title)
        expected = field_value(spec)
        matched = isinstance(resolution, int) and resolution == expected
...
def main() -> int:
    ...
    movies = api_get(api_key, "/api/v3/movie")
    queue_status = api_get(api_key, "/api/v3/queue/status")
    ...
    checks.append(
        (
            "native quality group covers 720/1080",
            grouped_resolutions == {720, 1080},
    ...
    movie_format = str(naming.get("standardMovieFormat") or "")
    checks.append(
        (
            "movie rename format preserves audio languages",
            "MediaInfo AudioLanguages" in movie_format,
            movie_format,
        )
    )
    checks.append(
        (
            "movie rename format preserves video codec",
            "MediaInfo VideoCodec" in movie_format,
            movie_format,
        )
    )
    checks.append(("Radarr renaming is enabled", bool(naming.get("renameMovies")), str(naming.get("renameMovies"))))
    ...
    movie_profile_counts: dict[str, int] = {}
    ...
    for item in movies:
        ...
        if name is None:
            ...
            name = str(profile_id)
        movie_profile_counts[name] = movie_profile_counts.get(name, 0) + 1
    checks.append(
        (
            "all movies reference known quality profiles",
            unknown_profile_count == 0,
            str(movie_profile_counts),
        )
    )

    print(f"Radarr profile: {ANIME_PROFILE_NAME} (id={profile.get('id')})")
    for label, passed, detail in checks:
        print(f"{'PASS' if passed else 'FAIL'}: {label} ({detail})")
    print(f"Radarr queue status: {queue_status}")
    print(f"movie profile counts: {movie_profile_counts}")

    samples = [
        ("JA+EN x265 1080p", "Akira (1988) [1080p] [JA+EN] [x265].mkv", True, True),
        ("EN+JA x265 1080p", "Akira (1988) [1080p] [EN+JA] [x265].mkv", True, True),
        ("JA+KO+EN x265 1080p", "Akira (1988) [1080p] [JA+KO+EN] [x265].mkv", True, True),
        ("KO+EN x265 1080p", "Akira (1988) [1080p] [KO+EN] [x265].mkv", True, True),
    ...
    print_header("Radarr Parse Smoke Checks")
    ...
    print_header("Result")
    if all_passed:
        print("PASS: live Radarr profile scores and title-side CF checks match expectations")
        return 0
    print("FAIL: one or more live Radarr expectations did not match")
    return 1
