...
REGULAR_ENGLISH_GUARD_CF_NAME = "Regular English - Foreign/Multi Audio Guard"
...
def positive_score_ceiling(scores: dict[str, int]) -> int:
    return sum(score for score in scores.values() if score > 0)
...
def audit_profile(
    ...
    if profile_check.kind == "anime":
        ...
        if profile_check.name in {"shows-anime-efficient", "movies-anime-efficient"}:
        ...
        regular_english_guard_score = scores.get(REGULAR_ENGLISH_GUARD_CF_NAME, 0)
        regular_english_profile = (
            "shows-regular-efficient"
            if instance.name == "sonarr"
            else "movies-regular-efficient"
        )
        if profile_check.name == regular_english_profile:
            if regular_english_guard_score > -10000:
                failures.append(
                    f"{profile_check.name}: {REGULAR_ENGLISH_GUARD_CF_NAME} "
                    f"must be a hard negative, got {regular_english_guard_score}"
                )
        elif regular_english_guard_score:
            failures.append(
                f"{profile_check.name}: {REGULAR_ENGLISH_GUARD_CF_NAME} "
                f"must stay unscored outside the English regular profile, got "
                f"{regular_english_guard_score}"
            )
    ...
    ceiling_score = positive_score_ceiling(scores)
    expected_cutoff_score = ceiling_score
    ...
    if actual_cutoff_score != expected_cutoff_score:
        failures.append(
            ...
            f"must equal exact positive CF ceiling {expected_cutoff_score}"
        )
    ...
    return {
        ...
        "regular_english_guard_score": scores.get(REGULAR_ENGLISH_GUARD_CF_NAME),
        ...
        "max_applicable_path_score": max_applicable_path_score,
        "positive_score_ceiling": ceiling_score,
    ...
    for instance in report["instances"]:
        ...
        for profile in instance["profiles"]:
            ...
            print(
                ...
                "cutoff={cutoff}/{expected_cutoff}; max path={max_path}".format(
                    ...
                    max_path=profile["max_applicable_path_score"],
                )
            )
