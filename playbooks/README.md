...
| `playbooks/agents/hermes-shadow.yml` | `hermes_hosts` | Stage three isolated Hermes profiles with root-owned behavior, Discord, and automation policy; native merged-config validation as each profile; approval-gated learning; root-writable profile-scoped managed environments; rootless Podman; and no production delivery or schedules; disabled by default. | hermes_shadow_*; templates/hermes; files/hermes; scripts/agents. | `ansible-playbook playbooks/agents/hermes-shadow.yml --syntax-check` |
