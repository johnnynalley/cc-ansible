...
| `playbooks/docker/agent-docker-report.yml` | `docker_hosts` | Publish a strict redacted Docker inventory through a forced-command account; disabled by default. | agent_docker_report_*; scripts/docker/agent-docker-report*. | `ansible-playbook playbooks/docker/agent-docker-report.yml --syntax-check` |
| `playbooks/docker/agent-docker-update-broker.yml` | `docker_hosts` | Install a strict, separately approved, digest-bound service update broker; disabled by default. | agent_docker_update_broker_*; templates/docker/agent-docker-update-manifest.json.j2; scripts/docker/agent-docker-update-broker.py. | `ansible-playbook playbooks/docker/agent-docker-update-broker.yml --syntax-check` |
...
| `playbooks/agents/hermes-shadow.yml` | `hermes_hosts` | Stage three isolated Hermes profiles with root-owned behavior, Discord, and automation policy; signed root-managed offline Tirith; native merged-config validation as each profile; approval-gated learning; root-writable profile-scoped managed environments; rootless Podman; and no production delivery or schedules; disabled by default. | hermes_shadow_*; hermes_tirith_*; templates/hermes; files/hermes; scripts/agents. | `ansible-playbook playbooks/agents/hermes-shadow.yml --syntax-check` |
