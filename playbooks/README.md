...
| Playbook | Hosts | Purpose | Main vars/sources | Safe validation |
| --- | --- | --- | --- | --- |
| `playbooks/media/immich-media-inbox.yml` | `docker_hosts` (opt-in) | Deploy Astra's headless Immich screenshot movie/TV review queue. | immich_media_inbox_*; templates/media-inbox; scripts/media-inbox. | `ansible-playbook playbooks/media/immich-media-inbox.yml --syntax-check` |
...
| `playbooks/agents/openclaw-isolated-gateway.yml` | `openclaw_hosts` | Stage a parallel deny-by-default Gateway canary; disabled by default. | openclaw_isolated_gateway_*; templates/openclaw; scripts/agents. | `ansible-playbook playbooks/agents/openclaw-isolated-gateway.yml --syntax-check` |
