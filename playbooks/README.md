...
| `playbooks/agents/openclaw-isolated-gateway.yml` | `openclaw_hosts` | Stage a two-phase deny-by-default Gateway canary with isolated OAuth enrollment and a required model proof; disabled by default. | openclaw_isolated_gateway_*; templates/openclaw; scripts/agents. | `ansible-playbook playbooks/agents/openclaw-isolated-gateway.yml --syntax-check` |
