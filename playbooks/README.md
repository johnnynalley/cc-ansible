...
| `playbooks/media/media-release-stamper.yml` | `gluetun_hosts` | Configure release metadata stamping plus exact-target import and terminal qBittorrent reconciliation; `--tags media-release-reconciler` scopes deployment to the reconciler boundary. | nightly_media_*, media_release_stamper_*, media_stack_health_*, stream_relay_*, plex_appliance_*; templates/media-maintenance, templates/streaming, templates/plex-appliance, templates/docker; scripts/media-maintenance, scripts/media-release, scripts/streaming. | `ansible-playbook playbooks/media/media-release-stamper.yml --syntax-check` |
...
| `playbooks/agents/hermes-production-runtime.yml` | `hermes_hosts` | Safely converge only rebuildable Hermes platform support, official messaging dependencies, functional Discord readiness, and native updater while leaving profile-authored guidance untouched; disabled by default. | hermes_production_runtime_*; hermes_shadow_*; templates/hermes; scripts/agents. | `ansible-playbook playbooks/agents/hermes-production-runtime.yml --syntax-check` |
...
| `playbooks/agents/hermes-lcm-backfill.yml` | `hermes_hosts` | Audit or install temporary staggered LCM summary/chunk historical-backfill timers. The units consume the profile-owned native `lcm.env`; Ansible owns only the external scheduler and bounded runner. | hermes_lcm_backfill_*; templates/hermes; scripts/agents. | `ansible-playbook playbooks/agents/hermes-lcm-backfill.yml --syntax-check` |
...
| `playbooks/agents/hermes-profile-skills.yml` | `hermes_hosts` | Retained one-time OpenClaw-to-Hermes skill migration transaction and evidence audit; disabled by default and never part of normal convergence or disaster recovery. Native profile skills are agent-owned and restored from the complete Hermes backup. | hermes_profile_skills_*; files/hermes/profile-skills-contract.json; files/hermes/profile-skills; scripts/agents/hermes-profile-skills-validate.py. | `ansible-playbook playbooks/agents/hermes-profile-skills.yml --syntax-check` |
