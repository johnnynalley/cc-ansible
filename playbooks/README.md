...
| `playbooks/agents/openclaw-doctor-rehearsal.yml` | `openclaw_hosts` | Rehearse supported Doctor migrations and replace legacy plugin install mechanisms on credential-free protected state copies; disabled by default. | openclaw_doctor_rehearsal_*; tasks/openclaw-doctor-snapshot.yml; scripts/agents/openclaw-doctor-rehearsal.py. | `ansible-playbook playbooks/agents/openclaw-doctor-rehearsal.yml --syntax-check` |
