...
| `media-release-stamper.yml` | `gluetun_hosts` | Configure media release metadata stamping; use `--tags media-release-reconciler` to deploy/reload only the exact-target reconciler. | `ansible-playbook playbooks/media/media-release-stamper.yml --syntax-check` |
