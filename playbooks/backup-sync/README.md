...
- Script owners: `scripts/agents/hermes-disaster-recovery-stage.py` for the
  opt-in Hermes application-consistent staging layer and
  `scripts/storage/restic-snapshot-freshness.py` for independent NAS-side
...
| `local-restic.yml` | `backup_clients:!macos_hosts, ts440` | Configure local Restic backups on TS440 ZFS, including stale-lock cleanup, bounded transient-outage retries, early skipping of disabled hosts, optional application-consistent Hermes disaster-recovery staging, and independent NAS-side freshness alerts. | `ansible-playbook playbooks/backup-sync/local-restic.yml --syntax-check` |
