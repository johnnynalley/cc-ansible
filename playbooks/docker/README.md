...
- Key vars: docker_stacks, docker_daemon_config, docker_auto_update_*,
  agent_docker_report_*, agent_docker_update_trigger_*, gluetun_*, qbit_*.
...
| `agent-docker-report.yml` | `docker_hosts` | Publish a strict redacted inventory through a dedicated forced-command account. | `ansible-playbook playbooks/docker/agent-docker-report.yml --syntax-check` |
| `agent-docker-update-trigger.yml` | `docker_hosts` | Expose only status and start for the existing Ansible-selected Docker auto-updater through a separate forced-command account. | `ansible-playbook playbooks/docker/agent-docker-update-trigger.yml --syntax-check` |
