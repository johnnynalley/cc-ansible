...
- The isolated Gateway resolves the stable core and Codex provider under a
  credential-less ephemeral build account with lifecycle scripts disabled,
  validates the pair, and atomically promotes a root-owned versioned release.
  Resolved release versions are rollback records, not update-policy pins.
...
| `openclaw-isolated-gateway.yml` | `openclaw_hosts` | Stage a modernized two-phase Gateway canary with immutable versioned core/provider releases, fresh OAuth enrollment, and a required model proof; disabled by default. | `ansible-playbook playbooks/agents/openclaw-isolated-gateway.yml --syntax-check` |
