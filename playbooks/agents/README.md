...
- Key vars: codex_*, claude_memory_sync_*, openclaw_*, hermes_shadow_*,
  hermes_profile_memory_*, hermes_profile_skills_*, hermes_automation_*.
...
| `hermes-profile-skills.yml` | `hermes_hosts` | Transactionally stage six reviewed declarative native skill deployments under root-owned per-profile sources, prove exact hashes and Hermes-native discovery through read-only service bindings, and leave every Gateway stopped; disabled by default. | `ansible-playbook playbooks/agents/hermes-profile-skills.yml --syntax-check` |
