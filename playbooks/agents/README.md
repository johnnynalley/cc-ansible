...
| `openclaw-isolated-gateway.yml` | `openclaw_hosts` | Stage a two-phase, deny-by-default Gateway canary under the `openclaw` account; bootstrap supports fresh OAuth enrollment and full canary requires a model proof; disabled by default. | `ansible-playbook playbooks/agents/openclaw-isolated-gateway.yml --syntax-check` |
