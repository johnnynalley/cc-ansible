...
| `hermes-shadow.yml` | `hermes_hosts` | Stage the isolated, boot-disabled Hermes runtime with three OS identities, root-owned identity/operating/Discord/automation contracts, a signed root-managed offline Tirith scanner, service-identity native config validation, Astra's validated hook-only Star privacy boundary, approval-gated learning, root-writable profile-scoped managed environments, and no production delivery or schedules; disabled by default. | `ansible-playbook playbooks/agents/hermes-shadow.yml --syntax-check` |
| `hermes-production-cutover.yml` | `hermes_hosts` | Transactionally stop and preserve OpenClaw, prove delivery state is drained, privately enroll the two existing Discord identities, promote Astra and Dubble for three logical roles, activate native silent Rigel scheduling and native updates, preserve Health, and automatically restore OpenClaw on any promotion failure; disabled by default. | `ansible-playbook playbooks/agents/hermes-production-cutover.yml --syntax-check` |
| `hermes-production-runtime.yml` | `hermes_hosts` | Back up and converge the live shared Hermes runtime, official Discord dependency/readiness boundary, native updater, and two production Gateway units one at a time while OpenClaw remains offline and Health remains active; disabled by default. | `ansible-playbook playbooks/agents/hermes-production-runtime.yml --syntax-check` |
| `hermes-native-gateway-migration.yml` | `hermes_hosts` | Copy retained flat profile state into native named profiles, replace handwritten base units through Hermes's native system installer, preserve hardened drop-ins, keep Rigel disabled, and restore the prior units/state on failure; disabled without exact approval. | `ansible-playbook playbooks/agents/hermes-native-gateway-migration.yml --syntax-check` |
...
| `hermes-automation.yml` | `hermes_hosts` | Back up and transactionally converge seven native jobs, retained collectors, calendar/feed timers, and profile backups; reconcile every legacy lane, keep OpenClaw offline, and restore files/timers on failure; disabled by default. | `ansible-playbook playbooks/agents/hermes-automation.yml --syntax-check` |
| `hermes-docker-inventory.yml` | `hermes_hosts` | Promote Astra's fixed-schema Docker inventory/status and approval-gated update trigger without Docker socket/group or general SSH; disabled without exact approval. | `ansible-playbook playbooks/agents/hermes-docker-inventory.yml --syntax-check` |
...
| `hermes-profile-skills.yml` | `hermes_hosts` | Transactionally stage seven reviewed declarative native skill deployments under root-owned per-profile sources, prove exact hashes and Hermes-native discovery through read-only service bindings, and leave every Gateway stopped; disabled by default. | `ansible-playbook playbooks/agents/hermes-profile-skills.yml --syntax-check` |
