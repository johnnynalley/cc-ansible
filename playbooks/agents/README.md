...
| `hermes-shadow.yml` | `hermes_hosts` | Stage the isolated, boot-disabled Hermes runtime with three OS identities, root-owned identity/operating/Discord contracts, approval-gated learning, root-writable profile-scoped managed environments, and no production delivery; disabled by default. | `ansible-playbook playbooks/agents/hermes-shadow.yml --syntax-check` |
